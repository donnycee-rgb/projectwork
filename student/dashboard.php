    ...
    async function loadData() {
      try {
        const [enrolledRes, feesRes] = await Promise.all([
          fetch(API + 'registrations.php?user_id=' + encodeURIComponent(window.DASH_USER_ID)).then((r) => r.json()),
          fetch(API + 'fees.php?action=student&id=' + encodeURIComponent(window.DASH_USER_ID)).then((r) => r.json())
        ]);
