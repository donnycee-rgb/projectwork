(function () {
  ...
  function pillCapacity(enrolled, capacity) {
    ...
    return (
      ...
      '<span class="pill-capacity-bar"><span class="pill-capacity-fill" style="width:' + pct + '%;background:' + fillColor(pct) + '"></span></span>' +
      '<span class="pill-capacity-text">' + enrolled + '/' + capacity + '</span></span>'
    );
  }
  ...
  function capacityListBar(enrolled, capacity) {
    const pct = capacityPercent(enrolled, capacity);
    return (
      '<div class="capacity-track"><div class="capacity-track-fill" style="width:' + pct + '%"></div></div>' +
  ...
  function capacityBar(enrolled, capacity) {
    ...
    return (
      '<div class="capacity-bar-wrap"><div class="capacity-bar"><div class="capacity-fill ' + cls + '" style="width:' + pct + '%"></div></div>' +
      '<span class="capacity-label">' + enrolled + '/' + capacity + '</span></div>'
    );
  }
  ...
  async function fetchCourses(admin) {
    return apiFetch(admin ? 'courses.php?action=admin' : 'courses.php');
  }
  ...
  async function ensureAdminSession() {
    ...
    try {
      ...
      if (!data.success) { window.location.href = '../login.html'; return false; }
      if (nameEl) nameEl.textContent = data.name;
      sessionStorage.setItem('gf_user_name', data.name);
      sessionStorage.setItem('gf_user_role', 'admin');
      return true;
    } catch {
      if (!cached) { window.location.href = '../login.html'; return false; }
      return true;
    }
  }
    ...
    try {
      ...
      if (coursesRes.success && list) {
        ...
        list.innerHTML = !courses.length
          ? '<p class="table-empty">No courses yet.</p>'
          : courses.map((c) =>
              '<div class="capacity-list-item">' +
              '<div class="capacity-list-name">' + escapeHtml(c.title) +
              '<span>' + escapeHtml(c.code) + ' · ' + escapeHtml(c.department) + '</span></div>' +
      ...
      if (recentRes.success && tbody) {
        ...
        tbody.innerHTML = !rows.length
          ? '<tr><td colspan="4" class="table-empty">No registrations yet.</td></tr>'
          : rows.map((r) =>
              '<tr><td>' + escapeHtml(r.first_name + ' ' + r.last_name) + '</td>' +
              '<td>' + escapeHtml(r.course_title) + '</td>' +
              '<td>' + escapeHtml(r.department) + '</td>' +
              '<td>' + formatDate(r.registered_at) + '</td></tr>'
            ).join('');
      }
    } catch {
      showSectionError('overviewError', 'Could not load overview data.');
    }
  ...
  function renderCoursesTable(courses) {
    ...
    if (!courses.length) {
      tbody.innerHTML = '<tr><td colspan="7" class="table-empty">No courses yet.</td></tr>';
      return;
    }
    tbody.innerHTML = courses.map((c) =>
      '<tr data-id="' + c.id + '" class="course-row">' +
      '<td>' + escapeHtml(c.code) + '</td>' +
      '<td><strong>' + escapeHtml(c.title) + '</strong></td>' +
      '<td>' + escapeHtml(c.department) + '</td>' +
      '<td>' + escapeHtml(c.instructor) + '</td>' +
      '<td>' + c.credits + '</td>' +
      '<td>' + pillCapacity(c.enrolled, c.capacity) + '</td>' +
      '<td class="course-actions">' +
      '<button type="button" class="btn-sm btn-outline btn-row-edit" data-id="' + c.id + '">Edit</button> ' +
      '<button type="button" class="btn-sm btn-danger btn-row-delete" data-id="' + c.id + '">Delete</button>' +
      '</td></tr>'
    ).join('');
  }
  ...
  function buildEditRow(course) {
    return (
      '<tr class="course-edit-row" data-id="' + course.id + '"><td colspan="7">' +
      '<form class="row-edit-form-inner" data-id="' + course.id + '">' +
      '<div class="row-edit-grid">' +
      '<div class="form-field"><label>Code</label><input name="code" value="' + escapeHtml(course.code) + '" required></div>' +
      '<div class="form-field"><label>Title</label><input name="title" value="' + escapeHtml(course.title) + '" required></div>' +
      '<div class="form-field"><label>Department</label><input name="department" value="' + escapeHtml(course.department) + '" required></div>' +
      '<div class="form-field"><label>Instructor</label><input name="instructor" value="' + escapeHtml(course.instructor) + '" required></div>' +
      '<div class="form-field"><label>Credits</label><input type="number" name="credits" min="1" max="12" value="' + course.credits + '" required></div>' +
      '<div class="form-field"><label>Capacity</label><input type="number" name="capacity" min="1" max="500" value="' + course.capacity + '" required></div>' +
      '<div class="form-field full"><label>Description</label><textarea name="description">' + escapeHtml(course.description || '') + '</textarea></div>' +
      '<div class="row-edit-actions">' +
      '<button type="submit" class="btn-primary btn-sm">Save</button>' +
      '<button type="button" class="btn-cancel-link btn-row-cancel">Cancel</button>' +
      '</div><p class="row-error" hidden></p>' +
      '</div></form></td></tr>'
    );
  }
  ...
  function initManageCoursesPage() {
    ...
    tbody?.addEventListener('click', async (e) => {
      ...
      if (delBtn) {
        ...
        cell.innerHTML =
          ...
          '<button type="button" class="confirm-yes btn-delete-yes" data-id="' + delBtn.dataset.id + '">Yes, delete</button> ' +
          '<button type="button" class="btn-cancel-link btn-delete-no" data-id="' + delBtn.dataset.id + '">Cancel</button></span>';
        return;
      }
      if (noBtn) { await loadCoursesPage(); return; }
      if (yesBtn) {
        ...
        try {
          ...
          if (data.success) {
            ...
            if (cell) {
              cell.innerHTML =
                '<span class="row-error">' + escapeHtml(data.message) + '</span> ' +
    ...
    tbody?.addEventListener('submit', async (e) => {
      ...
      try {
        const data = await editCourse(Object.assign({ id }, courseFormPayload(f)));
        if (data.success) {
        ...
        if (err) { err.textContent = 'Connection error.'; err.hidden = false; }
      }
    });
  ...
  function computeRegSummary(rows) {
    ...
    let popular = '—'; let max = 0;
    Object.keys(counts).forEach((k) => { if (counts[k] > max) { max = counts[k]; popular = k; } });
    return { total: rows.length, popular: max > 0 ? popular : '—', depts: depts.size };
  }
  ...
  function filterRegistrations(rows, search, dept) {
    ...
    return rows.filter((r) => {
      ...
      return (!q || name.includes(q) || course.includes(q)) && (!dept || r.department === dept);
    });
  }
  ...
  function sortRegistrations(rows) {
    return rows.slice().sort((a, b) => {
      let va, vb;
      switch (sortKey) {
        case 'student':      va = (a.first_name + ' ' + a.last_name).toLowerCase(); vb = (b.first_name + ' ' + b.last_name).toLowerCase(); break;
        case 'student_id':  va = a.student_id;    vb = b.student_id;    break;
        case 'course':      va = a.course_title;  vb = b.course_title;  break;
        case 'department':  va = a.department;    vb = b.department;    break;
        case 'credits':     va = parseInt(a.credits, 10); vb = parseInt(b.credits, 10); break;
        default:
  ...
  function renderRegTable(rows) {
    ...
    if (!tbody) return;
    tbody.innerHTML = !rows.length
      ? '<tr><td colspan="6" class="table-empty">No registrations match your filters.</td></tr>'
      : rows.map((r) =>
          '<tr><td>' + escapeHtml(r.first_name + ' ' + r.last_name) + '</td>' +
  ...
  function updateSortHeaders() {
    document.querySelectorAll('.data-table th.sortable').forEach((th) => {
      ...
      if (arrow) { arrow.classList.toggle('active', active); arrow.textContent = active ? (sortDir > 0 ? '▲' : '▼') : '↕'; }
    });
  }
  ...
  async function initRegistrationsPage() {
    ...
    try {
      ...
      if (!res.success) { showSectionError('regsError', res.message || 'Failed to load registrations.'); return; }
  ...
  async function loadStudentsSection() {
    ...
    try {
      ...
      if (!res.success) { showSectionError('studentsError', res.message || 'Failed to load students.'); return; }
      const rows = res.students || [];
      tbody.innerHTML = !rows.length
        ? '<tr><td colspan="6" class="table-empty">No students registered yet.</td></tr>'
        : rows.map((s) =>
            '<tr><td>' + escapeHtml((s.first_name || '') + ' ' + (s.last_name || '')) + '</td>' +
            '<td>' + escapeHtml(s.email) + '</td>' +
            '<td>' + escapeHtml(s.student_id) + '</td>' +
            '<td>' + (s.enrolled_count || 0) + '</td>' +
            '<td>' + formatKes(s.fees_paid) + '</td>' +
  ...
  async function loadFeesSection() {
    ...
    try {
      ...
      if (!res.success) { showSectionError('feesError', res.message || 'Failed to load fees.'); return; }
      if (costInput && res.cost_per_credit) costInput.value = parseFloat(res.cost_per_credit);
      const rows = res.students || [];
      feesStudentCache = {};
      tbody.innerHTML = !rows.length
        ? '<tr><td colspan="7" class="table-empty">No students found.</td></tr>'
        : rows.map((s) => {
            feesStudentCache[s.id] = s;
            return (
              '<tr data-id="' + s.id + '"><td>' + escapeHtml(s.name) + '</td>' +
              '<td>' + escapeHtml(s.student_id) + '</td>' +
              '<td>' + formatKes(s.fees_paid) + '</td>' +
              '<td>' + (s.credits_available || 0) + '</td>' +
              '<td>' + (s.credits_used || 0) + '</td>' +
              '<td>' + (s.credits_remaining || 0) + '</td>' +
              '<td><button type="button" class="btn-sm btn-outline btn-edit-fee" data-id="' + s.id + '">Update</button></td></tr>'
            );
          }).join('');
    } catch {
  ...
  function initFeesSection() {
    document.getElementById('feesSettingsForm')?.addEventListener('submit', async (e) => {
      e.preventDefault();
      showSectionError('feesError', '');
      ...
      try {
        ...
        if (data.success) { showToast(data.message || 'Setting saved.'); await loadFeesSection(); }
        else showSectionError('feesError', data.message || 'Could not save setting.');
      } catch { showSectionError('feesError', 'Connection error.'); }
    });
    ...
    document.getElementById('feeEditForm')?.addEventListener('submit', async (e) => {
      ...
      try {
        const data = await postJson('fees.php', { action: 'update', student_id: studentId, amount }, 'update');
        if (data.success) { showToast(data.message || 'Fees updated.'); closeFeeModal(); await loadFeesSection(); }
  ...
  // Expose loadAdmins globally so dashboard.php inline script can call it after creation
  async function loadAdminsSection() {
    showSectionError('adminsError', '');
    const tbody = document.getElementById('adminsTableBody');
    if (!tbody) return;

    // Get current logged-in admin id from session storage or PHP-set global
    const currentId = window.DASH_USER_ID || parseInt(sessionStorage.getItem('gf_user_id') || '0', 10);

    try {
      const res = await apiFetch('admins.php?action=all');
      if (!res.success) { showSectionError('adminsError', res.message || 'Failed to load admins.'); return; }
      const rows = res.admins || [];
      if (!rows.length) {
        tbody.innerHTML = '<tr><td colspan="5" class="table-empty">No admin accounts found.</td></tr>';
        return;
      }

      tbody.innerHTML = rows.map((a) => {
        const isYou      = a.id === currentId;
        const isProtected = a.id === 1 || isYou;

        const nameBadge  = isYou
          ? escapeHtml(a.name) + '<span class="admin-you-badge">You</span>'
          : escapeHtml(a.name);

        const actions = isProtected
          ? '<span style="font-size:.72rem;color:var(--text-lt)">Protected</span>'
          : '<button type="button" class="btn-reset-pwd" ' +
              'data-id="' + a.id + '" ' +
              'data-name="' + escapeHtml(a.name) + '" ' +
              'data-email="' + escapeHtml(a.email) + '" ' +
              'title="Generate new password for this admin">' +
              '<svg viewBox="0 0 24 24" style="width:12px;height:12px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;vertical-align:middle;margin-right:3px">' +
              '<path d="M1 4v6h6M23 20v-6h-6"/>' +
              '<path d="M20.49 9A9 9 0 005.64 5.64L1 10M23 14l-4.64 4.36A9 9 0 013.51 15"/></svg>' +
              'Reset Password</button> ' +
            '<button type="button" class="btn-sm btn-danger btn-delete-admin" data-id="' + a.id + '">Delete</button>';
        ...
        return (
          ...
          '<td>' + escapeHtml(a.created_by_name || 'System') + '</td>' +
          '<td>' + actions + '</td>' +
          '</tr>'
        );
      }).join('');
  ...
  // Make loadAdmins globally accessible for dashboard.php inline script
  window.loadAdmins = loadAdminsSection;
  ...
  function initAdminsSection() {
    ...
    // NOTE: form submit is now handled in dashboard.php inline script
    // which generates the password and shows the modal.
    ...
    tbody?.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (data.success) { showToast(data.message || 'Admin deleted.'); await loadAdminsSection(); }
  ...
  function initAdminSpa() {
    ...
    async function loadSection(sectionId) {
      ...
      else if (sectionId === 'section-courses')        initManageCoursesPage();
  ...
  function renderStudentEnrolled(courses) {
    ...
    tbody.innerHTML = !courses.length
      ? '<tr><td colspan="6" class="table-empty">You have not enrolled in any courses yet.</td></tr>'
      : courses.map((c) =>
          '<tr><td><strong>' + escapeHtml(c.title) + '</strong><br><span style="color:var(--text-lt);font-size:0.72rem">' + escapeHtml(c.code) + '</span></td>' +
          '<td>' + escapeHtml(c.department) + '</td>' +
          '<td>' + escapeHtml(c.instructor) + '</td>' +
          '<td>' + c.credits + '</td>' +
          '<td>' + capacityBar(c.enrolled, c.capacity) + '</td>' +
          '<td><button type="button" class="btn-sm btn-danger btn-unenroll" data-course-id="' + c.id + '">Unenroll</button></td></tr>'
        ).join('');
  }
  ...
  function renderStudentAvailable(courses) {
    ...
    if (!tbody) return;
    tbody.innerHTML = !courses.length
      ? '<tr><td colspan="6" class="table-empty">No additional courses available.</td></tr>'
      : courses.map((c) =>
          '<tr><td><strong>' + escapeHtml(c.title) + '</strong></td>' +
          '<td>' + escapeHtml(c.department) + '</td>' +
          '<td>' + escapeHtml(c.instructor) + '</td>' +
          '<td>' + c.credits + '</td>' +
          '<td>' + c.slots_remaining + '</td>' +
          '<td><button type="button" class="btn-enroll btn-enroll-course" data-course-id="' + c.id + '">Enroll</button></td></tr>'
        ).join('');
  }
  ...
  async function loadStudentDashboard() {
    try {
      const [statsRes, enrolledRes, availableRes] = await Promise.all([
        apiFetch('registrations.php?action=stats'),
        apiFetch('registrations.php?user_id=' + encodeURIComponent(window.DASH_USER_ID)),
        apiFetch('registrations.php?action=available'),
      ]);
      if (statsRes.success && statsRes.stats) {
  ...
  function initStudent() {
    document.getElementById('enrolledPanel')?.addEventListener('click', async (e) => {
      ...
      if (data.success) { showToast(data.message || 'Unenrolled.'); loadStudentDashboard(); }
    ...
    document.getElementById('availablePanel')?.addEventListener('click', async (e) => {
      ...
      if (data.success) { showToast(data.message || 'Enrolled.'); loadStudentDashboard(); }
      else { showToast(data.message || 'Failed.', true); btn.disabled = false; }
    });
  ...
  window.DashboardAPI = {
    fetchCourses, fetchRegistrations, fetchStats,
